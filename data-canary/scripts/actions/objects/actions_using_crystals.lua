...
function dangerousDepthCrystals.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local WarzoneVI = Position(1154, 826, 15)
	local WarzoneIV = Position(1159, 826, 15)
	local WarzoneV = Position(1164, 826, 15)
	local geodeId = 27510
		...
		if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneIV) < 30 then
			...
			if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneIV) == 30 then
				...
				if stalagmites then
					...
					addEvent(clearForgotten, 30 * 60 * 1000, Position(1122, 930, 15), Position(1147, 954, 15), Position(1159, 831, 15), Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneIV)
					startWarzoneIV()
				end
			end
		else
		...
		if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneV) < 30 then
			...
			if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneV) == 30 then
				...
				if stalagmites then
					...
					addEvent(clearForgotten, 30 * 60 * 1000, Position(1151, 963, 15), Position(1180, 982, 15), Position(1159, 831, 15), Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneV)
					startWarzoneV()
				end
			end
		else
		...
		if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneVI) < 30 then
			...
			if Game.getStorageValue(Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneVI) == 30 then
				...
				if stalagmites then
					...
					addEvent(clearForgotten, 30 * 60 * 1000, Position(1185, 930, 15), Position(1209, 954, 15), Position(1159, 831, 15), Storage.Quest.U11_50.DangerousDepths.Geodes.WarzoneVI)
					startWarzoneVI()
				end
			end
		else
