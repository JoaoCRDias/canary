...
local function doClearAreaAzerus()
	if Game.getStorageValue(Storage.Quest.Yalahar.LastFight) == 1 then
		...
		Game.setStorageValue(Storage.Quest.Yalahar.LastFight, 0)
	end
	return true
end
...
function inServiceYalaharLastFight.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	Game.setStorageValue(Storage.Quest.Yalahar.LastFight, 1)
