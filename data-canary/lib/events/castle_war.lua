local config = {
	...
	positions = {
		inside = { -- Castle Positions
			{fromPosition = Position(608, 1079, 5), toPosition = Position(625, 1104, 5)},
			{fromPosition = Position(610, 1080, 6), toPosition = Position(623, 1101, 6)},
			{fromPosition = Position(595, 1081, 7), toPosition = Position(637, 1111, 7)},
		},
		exitPosition = Position(1000, 1000, 7), -- Temple Position
	},
	throne = {
		position = Position(616, 1083, 5), -- Throne Position
	},
	areasSpawns = {
		{fromPosition = Position(596, 1080, 5), toPosition = Position(638, 1112, 7)},
	},
	timeThroneTick = 1,
	normalPontuation = 10,
	lastPontuation = 100,
	open = {
		...
		opentimes = {
			...
			[2] = "21:00:00", -- Monday
			[3] = "21:00:00", -- Tuesday
			[4] = "21:00:00", -- Wednesday
			[5] = "21:00:00", -- Thursday
			[6] = "21:00:00", -- Friday
			[7] = "21:00:00", -- Saturday
		},
		closein = 60, -- In minutes
	},
}
...
function Castle.open(self)
	...
	addEvent(function(self)
		self:close()
	end, config.open.closein * 60 * 1000, self)
	self.opened = true
	...
	addEvent(function(self)
		self:throne()
	end, config.timeThroneTick * 60 * 1000, self)
...
function Castle.throne(self)
	...
	if not guild then
		...
		playerTile:sendTextMessage(MESSAGE_EVENT_ADVANCE, "How did you get in here? It wasn't even supposed to be here.")
		return
	end
	...
	if next(playersCastle) ~= nil then
		for i = 1, #playersCastle do
			...
			if player then
				...
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "[Castle War] The castle was closed and you were teleported to temple.")
			end
		end
	end
...
function Castle.entraceCastle(self, playerId)
	...
	local dominant = self:getDominantGuild()
	if not guild then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You cannot enter Castle War without a Guild.")
		return false
	end
	...
	if (player:getLevel() < minLevel) then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format('You need to be at least level %d to join the Castle War.', minLevel))
		return false
	end

	if checkToEnter then
		if not self.opened then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The Castle is currently closed to raids.")
			return false
		end

		local membersOnline = guild:getMembersOnline()
		if (#membersOnline < config.join.minPlayers) then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format('You need to have %d online members in your Guild. Currently only owns %d.', config.join.minPlayers, #membersOnline))
			return false
		end
		...
		if (avaiableMembersToEnter < config.join.minPlayers) then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format('Your guild needs to have %d members online in your Guild above level %d. Currently only owns %d.', config.join.minPlayers, minLevel, avaiableMembersToEnter))
			return false
		end
...
function Castle.onTalkaction(self, player)
	if not self:isOpen() then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Castle War is currently closed.")
		return false
	end

	if not next(self.guildsInside) then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Currently no Guild has entered the Castle War yet.")
		return false
	end
